...
main :: IO ()
main = do
  hspec $ describe "src/Lexer.hs" $ do
    it "Atom" $
...
--   where
--     evalVal = unsafePerformIO $ runASTinEnv basicEnv $ fileToEvalForm "" expr
-- {-# NOINLINE tExpr #-}
...
--   where
--     evalVal = unsafePerformIO $ evalTextTest std file
-- {-# NOINLINE runExpr #-}
...
--   it (T.unpack note) $ evalVal `shouldBe` val
--   where
--     evalVal = unsafePerformIO $ evalExprTest expr
-- {-# NOINLINE tExprStd #-}
